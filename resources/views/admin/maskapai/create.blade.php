...
@section('content')
    <div class="py-5">
        <div class="w-full">
            ...
            <p>Tambah daftar maskapai penerbangan</p>
        </div>
        <form action="{{ route('maskapai.store') }}" method="POST" enctype="multipart/form-data" class="w-full flex flex-col space-y-5 mt-5">
            @csrf
            {{-- SINGLE INPUT --}}
            <div class="w-full">
                ...
                <div class="relative flex w-full flex-wrap mt-1">
                    <input type="file" name="logo_maskapai" class="py-3 placeholder-slate-400 text-slate-600 relative bg-white rounded text-sm focus:outline-none border-slate-300 focus:border-green-1000 w-full"/>
                    @error('logo_maskapai')
            ...
            <div class="w-full">
                ...
                <div class="relative flex w-full flex-wrap mt-1">
                    ...
                    <input type="text" name="nama_maskapai" placeholder="Nama maskapai" class="p-3 placeholder-slate-400 text-slate-600 relative bg-white rounded text-sm focus:outline-none border-slate-300 focus:border-green-1000 w-full pl-10"/>
                    @error('nama_maskapai')
                        <span class="text-sm italic text-red-500">{{ $message }}</span>
                    @enderror
                </div>
